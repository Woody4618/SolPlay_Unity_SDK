...
namespace SolPlay.CustomSmartContractExample
{
    ...
    public class HighscoreService : MonoBehaviour
    {
        ...
        {
            var solPlayNft = ServiceFactory.Instance.Resolve<NftService>().SelectedNft;
            if (solPlayNft != null)
            {
        ...
        public async Task SafeHighScore(SolPlayNft nft, uint highScore)
        {
            ...
            {
                ServiceFactory.Instance.Resolve<LoggingService>().Log("Block hash null. Connected to internet?", true);
                return;
            }
            ...
            SolPlayNft nft, uint highScore)
        {
            ...
            ServiceFactory.Instance.Resolve<LoggingService>().Log(sendingTransactionUsing, true);
            ...
            if (transactionSignature.WasSuccessful)
            {
                ...
                ServiceFactory.Instance.Resolve<LoggingService>().Log(checkingSignatureNow, true);

                CheckSignature(transactionSignature.Result, () =>
                {
                    GetHighscoreAccountData(nft);
                    ServiceFactory.Instance.Resolve<MessageRouter>().RaiseMessage(new SolBalanceChangedMessage());
                });
            }
            else
            {
                var error = $"There was an error: {transactionSignature.Reason} {transactionSignature.RawRpcResponse} ";
                ServiceFactory.Instance.Resolve<LoggingService>().LogWarning(error, true);
            }
        }
